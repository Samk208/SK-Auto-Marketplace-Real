...
export function AdminDealersClient({ initialDealers }: AdminDealersClientProps) {
    ...
    const handleVerify = async (id: string) => {
        ...
        try {
            ...
            toast({ title: "Dealer Verified", description: "Dealer can now operate fully." })
            router.refresh()
        } catch (error) {
            toast({ title: "Error", description: "Failed to verify dealer", variant: "destructive" })
        } finally {
            setIsUpdating(null)
        }
    }
    ...
    const handleRejectSubmit = async () => {
        ...
        try {
            const res = await fetch(`/api/admin/dealers/${selectedDealer.id}/verify`, {
                method: 'PATCH',
                body: JSON.stringify({ action: 'reject', notes: rejectReason })
            })
            if (!res.ok) throw new Error('Failed to reject')

            toast({ title: "Dealer Rejected", description: "Verification rejected." })
            setRejectDialogOpen(false)
            router.refresh()
        } catch (error) {
            toast({ title: "Error", description: "Failed to reject dealer", variant: "destructive" })
        } finally {
            setIsUpdating(null)
        ...
        <>
            <div className="overflow-x-auto">
                <table className="w-full">
                    ...
                    <tbody>
                        {dealers.map((dealer) => (
                            <tr key={dealer.id} className="border-b hover:bg-slate-50 dark:hover:bg-slate-800">
                                <td className="py-3 px-4">
                                    <div className="font-medium">{dealer.business_name}</div>
                                    <div className="text-xs text-slate-500">{dealer.location || 'No Location'}</div>
                                </td>
                                <td className="py-3 px-4">
                                    <div className="flex flex-col gap-1 text-sm">
                                        ...
                                        <div className="flex items-center gap-1">
                                            <Phone className="h-3 w-3" /> {dealer.users?.phone || 'N/A'}
                                        </div>
                                    </div>
                                </td>
                                <td className="py-3 px-4">
                                ...
                                <td className="py-3 px-4">
                                    <div className="flex flex-col gap-1 items-start">
                                        ...
                                        {dealer.business_license_url && (
                                            <Button variant="link" size="sm" className="h-6 px-0 text-blue-600" onClick={() => handleViewLicense(dealer.business_license_url!)}>
            ...
            <Dialog open={rejectDialogOpen} onOpenChange={setRejectDialogOpen}>
                <DialogContent>
                    ...
                    <DialogFooter>
                        ...
                        <Button variant="destructive" onClick={handleRejectSubmit} disabled={!rejectReason}>Reject</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
        </>
