...
export async function calculateTrustScoreAction(
  listingId: string,
  ...
  const verifiedDocuments: VerifiedDocument[] = [
    ...
    },
    {
      ...
      verifiedDate: listing.inspection_report_url
        ? new Date().toISOString().split("T")[0]
        : undefined,
    },
    {
  ...
  const trustFactors: TrustFactor[] = [
    {
      ...
      description: dealer?.verified
        ? "Dealer identity verified by SK AutoSphere"
    ...
    },
    {
      ...
      impact: "high",
      description: hasAiReport
        ? `AI scored this vehicle ${aiScore}/100`
        : "No AI condition report generated yet",
    },
    {
      name: "Inspection Valid",
      status: listing.inspection_report_url ? "verified" : "missing",
      impact: "high",
      description: listing.inspection_report_url
    ...
    },
    {
      ...
      impact: "medium",
      description: listing.service_history
  ...
  if (!listing.inspection_report_url)
    warnings.push("Missing inspection report");
...
export async function estimateDetailedPricingAction(
  ...
  const comparableListings: ComparableListing[] = (listings || []).map(
    (l: any) => ({
      id: l.id,
      make: l.brand,
      model: l.model,
  ...
  try {
    ...
    const avgPrice =
      comparableListings.length > 0
        ? comparableListings.reduce((sum, l) => sum + l.price, 0) /
    ...
    if (avgPrice > 0) {
      return {
        ...
        reasoning: [
          ...
          "AI service unavailable temporarily",
        ],
