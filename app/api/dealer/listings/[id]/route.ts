...
import { updateListingForDealer, deleteListingForDealer, getListingByIdForDealer } from '@/lib/repositories/listings'
...
const updateListingSchema = z.object({
  ...
  body_type: z.enum(['Sedan', 'SUV', 'Truck', 'Van', 'Coupe', 'Wagon', 'Bus', 'Other']).optional(),
...
export async function GET(
  ...
  try {
    ...
    if (!listing) {
      return NextResponse.json(
        { error: 'Not Found', message: 'Listing not found or you do not have permission to view it' },
        { status: 404 }
      )
    }
...
export async function PATCH(
  ...
  try {
    ...
    if (!dealer) {
      return NextResponse.json(
        { error: 'Forbidden', message: 'You must be a registered dealer to update listings' },
        { status: 403 }
      )
    }
    ...
    // 4. Update listing
    const listing = await updateListingForDealer(id, dealer.id, validationResult.data)
...
export async function DELETE(
  ...
  try {
    ...
    if (!dealer) {
      return NextResponse.json(
        { error: 'Forbidden', message: 'You must be a registered dealer to delete listings' },
        { status: 403 }
      )
    }
