...
import {
  getDealerEmail,
  sendDealerRefundNotificationEmail,
...
export async function POST(
  ...
  try {
    ...
    const { error: updateError } = await (
      supabaseServer.from("transactions") as any
    )
      .update({
        status: "refunded",
        metadata: {
    ...
    if (txData.listing_id) {
      const { error: listingError } = await (
        supabaseServer.from("car_listings") as any
      )
      ...
      if (listingError) {
        console.error(
          "[Refund] Failed to update listing status:",
          listingError,
        );
      } else {
