/**
 * !CRITICAL WARNING: PATTERN 2 DEPLOYMENT
 *
 * This route is the "Direct Next.js" WhatsApp implementation.
 *
 * IF YOU ARE USING "PATTERN 2" (n8n as the single ingestion point),
 * YOU MUST NOT REGISTER THIS URL IN THE META DEVELOPER DASHBOARD.
 *
 * Doing so will cause DOUBLE REPLIES (one from n8n, one from here) and race conditions.
 *
 * Refer to: docs/whatspp n8n/WHATSAPP_N8N_CHATWOOT_PATTERN2_HANDOVER.md
 *
 * If you want to use this route instead of n8n, ensure n8n is disabled.
 */
...
async function sendMessageToWhatsApp(to: string, text: string) {
  ...
  if (!token || !phoneId) {
    console.error(
      "Missing WhatsApp credentials (WHATSAPP_API_TOKEN or WHATSAPP_PHONE_ID)",
    );
    return;
  }
  ...
  try {
    ...
    if (body.object) {
      if (
        ...
        if (messageBody) {
          console.log(
            `[WhatsApp] Received message from ${name} (${phoneNumber}): ${messageBody}`,
          );
          ...
          // 4. Get AI Response
          const aiResponse = await chatWithGemini(messageBody, contextHistory, {
            username: name,
          });
          ...
          console.log(
            `[WhatsApp] Sending response to ${phoneNumber}: ${aiResponse}`,
          );
