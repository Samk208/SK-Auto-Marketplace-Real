    ...
    try {
      const response = await fetch("/api/ai/condition-report", {
        ...
        body: JSON.stringify({
          ...
          vehicleInfo: {
            make: selectedImage.label.includes("Damage")
              ? "Hyundai"
              : "Genesis",
            model: selectedImage.label.includes("Damage") ? "Sonata" : "G70",
          },
        }),
      });
      ...
      if (data.success && data.flags) {
        // Update selected image with real results for this session
        const realIssues = data.flags
          .filter((f: any) => f.severity !== "info")
          .map((f: any) => ({
            type: f.category,
            location: f.description.split(":")[0] || "General",
            severity: f.severity.charAt(0).toUpperCase() + f.severity.slice(1),
          }));

        const realStatus = realIssues.length > 0 ? "Damage Detected" : "Clean";
        const realConfidence = data.flags[0]?.confidence || 95;

        // Create a temporary update for the selected image
        const updatedResult = {
          status: realStatus,
        ...
        // We override the display temporarily
      ...
      console.error("[Vision Demo] Real scan failed, using mock data:", error);
    } finally {
      setIsScanning(false);
      setShowResult(true);
    }
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div>
            ...
            <p className="text-slate-400 text-lg mb-8 leading-relaxed">
              Our AI scans every listing for hidden damage, dents, and repainted
              panels. Try it now on these sample vehicles to see the power of SK
              AutoSphere Vision™.
            </p>
                ...
                {isScanning && (
                  <div className="absolute inset-0 flex items-center justify-center">
                    ...
                    <div className="flex flex-col items-center gap-3 z-10">
                      ...
                      <span className="text-blue-200 font-mono text-sm tracking-wider">
                        ANALYZING GEOMETRY...
                      </span>
                    </div>
                  </div>
                    ...
                    <div className="w-full max-w-sm bg-slate-900 rounded-xl border border-slate-700 p-6 shadow-xl">
                      ...
                      <div className="space-y-3 mb-6">
                        {selectedImage.result.issues.length > 0 ? (
                          selectedImage.result.issues.map(
                            (issue: any, i: number) => (
                              <div
                                key={i}
                                className="flex items-center justify-between text-sm p-2 rounded bg-slate-800"
                              >
