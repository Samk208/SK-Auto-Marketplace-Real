...
export function AiFeaturesSectionV2() {
  // Animation Variants - Implementing "Wave Logic" and "High-Tension Easing"
  const containerVariants: Variants = {
    hidden: { opacity: 0 },
    visible: {
      opacity: 1,
      transition: {
        staggerChildren: 0.1, // Temporal Staggering (50ms-100ms range)
        delayChildren: 0.1,
      },
    },
  };
  ...
  const productionAgents: AgentProps[] = [
    {
  ...
  const roadmapAgents: AgentProps[] = [
    {
      ...
      <div className="container mx-auto px-4">
        ...
        >
          <motion.div
            ...
            className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-sm font-bold uppercase tracking-widest mb-6"
          >
            {/* Pulsing Effect on Icon */}
            <div className="relative">
              <Sparkles size={16} className="relative z-10" />
              <div className="absolute inset-0 bg-blue-500/50 blur-[8px] animate-pulse" />
            </div>
          ...
          <motion.h2
            variants={itemVariants}
            className="text-3xl md:text-5xl font-black mb-6 text-slate-900 dark:text-white tracking-tight"
          >
            AI-Powered{" "}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-7xl mx-auto">
            {productionAgents.map((agent, i) => (
              <Link key={i} href={agent.href} className="block h-full">
                <motion.div
                  variants={itemVariants}
                  whileHover={{
                    y: -8,
                  ...
                  transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }} // Kinetic Feedback
                  className="h-full"
                >
                  <Card className="h-full border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 transition-colors relative overflow-hidden group">
                    <div className="absolute top-0 w-full h-1 bg-gradient-to-r from-green-400 to-emerald-500"></div>
                    <CardContent className="p-8">
                      <div className="flex justify-between items-start mb-6">
                        <div
                          className={`w-14 h-14 rounded-2xl flex items-center justify-center ${agent.bg} ${agent.color} transition-transform group-hover:scale-110 duration-500`}
                        >
                          <agent.icon size={28} />
                        </div>
                        <span
                          className={`px-3 py-1 rounded text-[10px] font-bold tracking-wider ${agent.badgeColor} shadow-md shadow-green-500/20`}
                        >
                          {agent.badge}
                        </span>
                      </div>
            ...
            {roadmapAgents.map((agent, i) => (
              <Link key={i} href={agent.href} className="block h-full">
                <motion.div
                  ...
                  className="h-full"
                >
                  <Card className="h-full border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 border-dashed opacity-80 hover:opacity-100 transition-opacity duration-300 hover:shadow-lg">
        ...
        >
          <motion.div variants={itemVariants}>
            <Button
              asChild
              size="lg"
              className="h-14 px-8 text-lg rounded-full bg-slate-900 text-white hover:bg-slate-800 shadow-xl shadow-blue-900/20 transition-all"
            >
