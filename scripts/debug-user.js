...
async function checkUser() {
    console.log("🔍 Checking valid users in 'auth.users' vs 'users' (public table)...");
    ...
    if (error) {
        console.log("No profile found in public.profiles. Checking public.users...");
        ...
        if (error2) {
            ...
            console.log("💡 We probably need to create a public profile for this auth user first.");
...
async function createProfile(authUser) {
    ...
    if (error1) {
        console.log("Failed to insert into 'users', trying 'profiles'...", error1.message);
        ...
        if (error2) {
            console.error("Failed to create profile in either table.", error2.message);
        } else {
