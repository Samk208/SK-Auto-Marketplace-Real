...
if (!env.NEXT_PUBLIC_SUPABASE_URL || !env.SUPABASE_SERVICE_ROLE_KEY) {
  ...
  console.log('\nCurrent values:');
  console.log('  NEXT_PUBLIC_SUPABASE_URL:', env.NEXT_PUBLIC_SUPABASE_URL ? '✅ Found' : '❌ Missing');
...
async function runSecurityTests() {
    ...
    if (error) {
      ...
      if (adminUsers.length > 0) {
        ...
        adminUsers.forEach(admin => {
          ...
          console.log(`     app_metadata.role: ${admin.app_metadata?.role || 'NOT SET'}`);
          console.log(`     user_metadata.role: ${admin.user_metadata?.role || 'NOT SET'}`);
        });
        passedTests++;
      } else {
        console.log('❌ FAILED: No admin users found');
        console.log('   Action needed: Run scripts/set-admin.js to create admin user');
        failedTests++;
      }
    }
  } catch (error) {
  ...
  try {
    ...
    if (adminError) {
      ...
      console.log(`✅ PASSED: audit_logs table exists (${adminLogs?.length || 0} recent records)`);
      passedTests++;
    }
    ...
    if (anonError || !anonLogs || anonLogs.length === 0) {
      console.log('✅ PASSED: Anonymous users cannot read audit_logs (RLS working)');
      passedTests++;
    } else {
      console.log('❌ FAILED: Anonymous users CAN read audit_logs (RLS BROKEN!)');
      console.log('   SECURITY RISK: Audit logs should only be readable by admins');
      failedTests++;
    }
  ...
  try {
    ...
    if (!pendingListings || pendingListings.length === 0) {
      console.log('✅ PASSED: Public cannot view pending listings (RLS working)');
      passedTests++;
    } else {
      console.log('❌ FAILED: Public CAN view pending listings (RLS BROKEN!)');
      console.log('   SECURITY RISK: Only admins/dealers should see pending listings');
      failedTests++;
    }
  ...
  try {
    ...
    if (pendingListing) {
      ...
      if (successful === 1 && failed === 1) {
        console.log('✅ PASSED: Concurrency protection works (only 1 update succeeded)');
        passedTests++;
      } else {
        console.log(`❌ FAILED: Both updates succeeded (${successful}) or both failed (${failed})`);
      ...
      console.log('   Action: Create a pending listing to test concurrency protection');
    }
