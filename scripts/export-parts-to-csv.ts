...
async function exportParts() {
  ...
  if (newParts.length > 0) {
    fs.writeFileSync(
      path.join(outputDir, "SUPABASE_NEW_PARTS.csv"),
      partsToCSV(newParts as Part[]),
    );
  ...
  if (usedParts.length > 0) {
    fs.writeFileSync(
      path.join(outputDir, "SUPABASE_USED_PARTS.csv"),
      partsToCSV(usedParts as Part[]),
    );
    console.log(
      `Exported: SUPABASE_USED_PARTS.csv (${usedParts.length} parts)`,
    );
  }

  if (machineParts.length > 0) {
    fs.writeFileSync(
      path.join(outputDir, "SUPABASE_MACHINE_PARTS.csv"),
      partsToCSV(machineParts as Part[]),
    );
    console.log(
      `Exported: SUPABASE_MACHINE_PARTS.csv (${machineParts.length} parts)`,
    );
  }
