...
async function checkDatabaseStatus() {
  ...
  try {
    ...
    const { data: dealers, error: dealersError } = await supabase
      .from("dealers")
      .select("id, business_name, verified, active_listings", {
        count: "exact",
      });
    ...
    if (dealersError) {
      ...
      dealers?.forEach((d) =>
        console.log(`   - ${d.business_name} (Active: ${d.active_listings})`),
      );
    }
    ...
    if (auditError) {
      console.log(
        "❌ audit_logs table: NOT FOUND or error -",
        auditError.message,
      );
    } else {
      console.log(
        `✅ audit_logs table: ${auditLogs?.length || 0} recent records`,
      );
    }
    ...
    console.log(
      anonListings
        ? "✅ RLS: Public can view active listings"
        : "❌ RLS: Public access blocked",
    );
    ...
    const admins = adminUsers?.users?.filter(
      (u) =>
