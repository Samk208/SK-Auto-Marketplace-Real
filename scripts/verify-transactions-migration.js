...
async function verifyMigration() {
  ...
  if (!authError) {
    ...
    warnings.push(
      `⚠️  Cannot verify auth.users access (expected in migration context)`,
    );
  }
  ...
  if (!txError) {
    warnings.push(
      "⚠️  transactions table ALREADY EXISTS - migration may conflict",
    );
    ...
    console.log(
      "   ✅ transactions table does not exist (ready for migration)",
    );
  } else {
    warnings.push(
      `⚠️  Unexpected error checking transactions: ${txError.message}`,
    );
  }
