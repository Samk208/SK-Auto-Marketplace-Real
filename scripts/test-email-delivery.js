...
async function testEmailDelivery() {
  ...
  console.log(
    ...
    hasSiteUrl ? `✅ ${env.NEXT_PUBLIC_SITE_URL}` : "❌ Missing",
  );
  ...
  try {
    ...
    if (!adminUser) {
      ...
      console.log(
        "   Action: Create an admin user first (run scripts/set-admin.js)",
      );
      return;
    }
    ...
    console.log("");
    console.log(
      "   OR use this curl command (replace TOKEN with your session token):",
    );
    console.log("");
    console.log(
      `   curl -X PATCH ${baseUrl}/api/admin/listings/${pendingListing.id}/approve \\`,
    );
  ...
  console.log("-".repeat(60));
  console.log(
    "   Similar to approval test, this requires manual testing or authenticated API call",
  );
  console.log("");
  console.log("   📝 MANUAL TEST INSTRUCTIONS:");
