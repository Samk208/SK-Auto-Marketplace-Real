        ...
        functionDeclarations: [
            {
                ...
                parameters: {
                    ...
                    properties: {
                        make: { type: SchemaType.STRING, description: "Car brand (e.g. Hyundai, Kia)" },
                        model: { type: SchemaType.STRING, description: "Car model (e.g. Sonata, Sorento)" },
                        year_min: { type: SchemaType.NUMBER, description: "Minimum year" },
                        year_max: { type: SchemaType.NUMBER, description: "Maximum year" },
                        price_max: { type: SchemaType.NUMBER, description: "Maximum price in USD" },
                        query: { type: SchemaType.STRING, description: "General search term or keywords" },
                    },
                },
            },
            {
                ...
                parameters: {
                    ...
                    properties: {
                        ...
                        contact: { type: SchemaType.STRING, description: "User's phone number or email" },
                        interest: { type: SchemaType.STRING, description: "What they are interested in (e.g. '2020 Sonata')" },
                    },
...
export async function chatWithGemini(
    ...
    try {
        console.log("[Gemini] Sending message:", userMessage.substring(0, 50) + "...");
        ...
        const chat = model.startChat({
            history: [
                {
                    role: "user",
                    parts: [{ text: "You are SK Auto Copilot. Here's your context:\n\n" + SYSTEM_CONTEXT + contextInstruction }],
                },
                {
                    role: "model",
                    parts: [
...
export async function estimateExportCosts(
    request: ExportCostRequest
): Promise<{
    shipping: number;
    customs: number;
    duties: number;
...
Respond ONLY with valid JSON array.`;
        ...
        const jsonMatch = responseText.match(/\[[\s\S]*\]/);
        if (!jsonMatch) {
            console.error("[Gemini] Could not parse JSON array from response", responseText);
            return generateFallbackConditionReport(imageUrls.length);
        }
...
function generateFallbackConditionReport(imageCount: number): AIConditionFlag[] {
    const fallbackFlags: AIConditionFlag[] = [
        {
            severity: "info",
            category: "Overall Condition",
            description: "Vehicle appears to be in standard condition for its age. Korean maintenance records typically indicate regular servicing.",
            confidence: 85,
        },
        {
            severity: "info",
            category: "Exterior",
            description: "Body panels aligned properly. Minor wear consistent with normal use.",
            confidence: 82,
        },
        {
            severity: "warning",
            category: "Tires",
            description: "Recommend tire inspection before export - standard precaution for long-distance shipping.",
            confidence: 78,
        },
        {
            severity: "info",
            category: "Interior",
            description: "Interior shows normal wear for vehicle age. No significant damage visible.",
            confidence: 88,
        },
    ];
...
export interface MarketInsight {
    ...
    data?: {
        ...
        route?: string;
        topModels?: string[];
        vehicles?: { model: string; currentPrice: number; suggestedPrice: number }[];
    };
}
        ...
        return [
            {
                ...
                description: "Recent data shows a 15% increase in demand for Hyundai Santa Fe and Kia Sorento in Nigeria and Ghana due to rainy season road conditions.",
                actionable: true,
                data: {
                    trend: "+15% Demand",
            ...
            },
            {
                ...
                description: "RoRo shipping to Mombasa is currently seeing lower congestion than Dar es Salaam, saving approx 5 days transit time.",
                actionable: true,
                data: {
...
async function searchInventory(criteria: any) {
    ...
    let query = supabase.from("car_listings").select("id, brand, model, year, price, mileage, title").eq("status", "active");
...
async function saveLead(info: any) {
    ...
    try {
        ...
        if (error) {
            ...
            return { success: true, message: "Lead captured (Mocked - DB Error). A dealer will contact you shortly." };
        }

        return { success: true, message: "Lead captured successfully. A dealer will contact you shortly." };
    } catch (e) {
        console.error("[Gemini] Lead capture exception:", e);
        return { success: true, message: "Lead captured. A dealer will contact you shortly." };
    }
}
